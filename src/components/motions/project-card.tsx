        ...
        <ScrollFade direction={textAnimDir}>
          ...
          <div className="flex flex-wrap gap-2 pt-4">
            {tags.map((tag) => (
              <span 
                key={tag} 
                className="px-3 py-1 bg-primary/10 dark:bg-primary/20 text-primary font-mono text-xs md:text-sm rounded-sm border border-primary/20 transition-all duration-500"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="flex items-center gap-8 pt-8 font-semibold">
            {repoLink ? (
              <a href={repoLink} target="_blank" className="flex items-center gap-2 text-foreground hover:text-primary transition-colors duration-500">
                <Code2 size={20} /> Code
              </a>
            ) : (
